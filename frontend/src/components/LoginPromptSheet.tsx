...
export default function LoginPromptSheet({ onClose }: LoginPromptSheetProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="mb-1 h-1 w-10 rounded-full bg-zinc-600 mx-auto" />
        <h2 className="mt-4 text-center text-lg font-bold">로그인이 필요해요</h2>
        <p className="mt-1 text-center text-sm text-zinc-400">
          좋아요를 누르려면 로그인하세요
        </p>
        <button
          onClick={() => navigate('/login')}
          className="mt-5 w-full rounded-xl bg-bitcoin py-3 font-semibold text-black"
        >
          로그인 / 회원가입
        </button>
