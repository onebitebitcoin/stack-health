...
def _reg(client: TestClient, email: str = "c@x.com", username: str = "cuser") -> str:
...
def _make_post(client: TestClient, token: str) -> int:
    with patch("app.routes.videos.r2_service.get_cdn_url", return_value="https://cdn/v.mp4"):
        res = client.post("/api/v1/videos/confirm", json={"r2_key": "videos/v.mp4", "duration_sec": 20}, headers=_auth(token))
    return res.json()["data"]["post"]["id"]


def test_list_comments_empty(client: TestClient) -> None:
    token = _reg(client)
    post_id = _make_post(client, token)
...
def test_create_comment_requires_auth(client: TestClient) -> None:
    token = _reg(client, "ca@x.com", "cusera")
    post_id = _make_post(client, token)
...
def test_create_comment_success(client: TestClient) -> None:
    token = _reg(client, "cb@x.com", "cuserb")
    post_id = _make_post(client, token)
...
def test_create_comment_appears_in_list(client: TestClient) -> None:
    token = _reg(client, "cc@x.com", "cuserc")
    post_id = _make_post(client, token)
...
def test_create_comment_too_long(client: TestClient) -> None:
    token = _reg(client, "cd@x.com", "cuserd")
    post_id = _make_post(client, token)
    res = client.post(
...
def test_create_comment_empty_content(client: TestClient) -> None:
    token = _reg(client, "ce@x.com", "cusere")
    post_id = _make_post(client, token)
...
def test_delete_comment_by_owner(client: TestClient) -> None:
    token = _reg(client, "cf@x.com", "caserf")
    post_id = _make_post(client, token)
...
def test_delete_comment_by_other_user_forbidden(client: TestClient) -> None:
    token_owner = _reg(client, "cg@x.com", "cuserg")
    token_other = _reg(client, "ch@x.com", "cuserh")
    post_id = _make_post(client, token_owner)
